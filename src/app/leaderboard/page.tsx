...
export default async function LeaderboardPage() {
  const [bullet, blitz, rapid, classical] = await Promise.all([
    db
      ...
      .from(users).where(not(like(users.username, "Stockfish%"))).orderBy(desc(users.bulletRating)).limit(100),
    db
      ...
      .from(users).where(not(like(users.username, "Stockfish%"))).orderBy(desc(users.blitzRating)).limit(100),
    db
      ...
      .from(users).where(not(like(users.username, "Stockfish%"))).orderBy(desc(users.rapidRating)).limit(100),
    db
      ...
      .from(users).where(not(like(users.username, "Stockfish%"))).orderBy(desc(users.classicalRating)).limit(100)
  ]);
